...
export default function ImpactSaleMMads() {
  ...
  const mapRow = (row, campaign) => {
    const actionEarning = parseFloat(row["commission"]);

    if (
      (campaign.Id === 2537 && campaign.name === "Hume Health") ||
      (campaign.Id === 2537 && campaign.name === "Hume Health US")
    ) {
      return {
        p1: row["click_ref2"],
        created: row["date"],
        ...
        status: "Pending",
        // sub1: row["Sub Id 1"],
        device_id: row["transaction_device"] || "unknown",
      };
    }
  };
    ...
    if (extension === "xlsx") {
      reader.onload = (evt) => {
        ...
        const cleaned = jsonData
          .filter((row) => row.site_name && row.site_name.trim() !== "")
        ...
        uniqueBrands.forEach((brand) => {
          ...
          brandWise[brand] = brandRows.map((row) => mapRow(row, config));
        });
  ...
  return (
    <div style={{ padding: "20px" }}>
      <h2>📁 Upload HumeHealthClickOrbit Report</h2>
