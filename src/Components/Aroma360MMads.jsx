...
export default function Aroma360MMads() {
  const [rawData, setRawData] = useState([]);
  const [brands, setBrands] = useState([]);
  const [groupedData, setGroupedData] = useState({});
  const [customFileName, setCustomFileName] = useState("");

  const mapAromaRow = (row, campaign) => {
    if (campaign.Id === 2564 && campaign.name === "Aroma360") {
      return {
        created: (() => {
        ...
        txn_id: row["Conversion ID"],
        sale_amount: row["Order Total"],
        revenue: row["Commission Total"],
        payout: ((row["Commission Total"] * 80) / 100).toFixed(10),
        ...
        Papa.parse(text, {
          ...
          complete: (results) => {
            ...
            const cleaned = csvData.filter(
              (row) => parseFloat(row["Order Total"] || 0) > 0,
            );
            setRawData(cleaned);
            ...
            uniqueBrands.forEach((brand) => {
              const brandRows = cleaned.filter((row) => "Aroma360" === brand);
  ...
  const handleDownloadCSV = (brand) => {
    ...
    const dates = data
      .map((row) => parseAromaDate(row.created))
      .filter(Boolean);
  ...
  return (
    <div style={{ padding: "20px" }}>
      <h2>📁 Upload Aroma360 MMads Report</h2>
